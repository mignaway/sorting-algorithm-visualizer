...
function App() {
  ...
  const sorting_algorithms = { bubble: 'Bubble sort', insertion: 'Insertion sort', selection: 'Selection sort', }
  ...
  useEffect(() => {
    if (isCurrentlySorted) generateArrayOfSize(generateArraySize)
  }, [current_sorting])
  ...
  const changeSortingSpeedHandler = (goNext) => {
    ...
    const currentElementIndex = multipliers.indexOf(sortingSpeedMultiplier)
  ...
  const sortingStop = () => {
    ...
    // Temp fix because i can't get updated state "sortingActive" in the sorting functions to stop it
  ...
  // const testGetSortingActive = () => {
  //   return sortingActive
  // }
  ...
  const bubble_sort_algorithm = async () => {
    ...
    for (let i = 0; i < temp_array.length ; i++){
      ...
      for (let j = 0; j < temp_array.length; j++) {
        ...
        if (!sortingActive) {console.log("stopped"); return false }
        ...
        await delay(SORTING_ARRAY_SPEED / sortingSpeedMultiplier);
        setArray([...temp_array]);
      }
